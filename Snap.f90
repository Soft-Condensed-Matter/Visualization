...
MODULE POVAR
 ...
 INTEGER, PARAMETER:: NPARTX = 5000         !MAXIMUM NUMBER OF PARTICLES
 ...
 INTEGER:: NP,ISAM                          !NUMBER OF PARTICLES
...
PROGRAM SNAPSHOT
 ...
 PHI=0.025D0                                !PACKING FRACTION
 RHOSTAR=6.0D0*PHI/PI                       !DENSITY NUMBER
 BOXX=(DBLE(NP)/RHOSTAR)**(1.0D0/3.0D0) + 0.1D0
 RC=0.5D0*BOXX
 L=RC + 0.05D0*SIGMA

 OPEN(UNIT=10,FILE='MDFin.dat',STATUS='OLD')
 OPEN(UNIT=11,FILE='Snapshot.pov',STATUS='UNKNOWN')

 DO I=1,NP
    READ(10,*)RX(I),RY(I),RZ(I)             !READ DATA FROM FILE
 ENDDO
...
!LIGHT SOURCE AND CAMERA POSITION
 ...
 WRITE(11,*) "camera{"
 WRITE(11,*) "location <",1.750*BOXX,",",BOXX,",",BOXX,">"
 WRITE(11,*) "look_at <0, 0, 0>"
 WRITE(11,*) "angle 70}"
 WRITE(11,*) "light_source { <30, 25, 20> color White }"
 WRITE(11,*) "light_source { <30, 25, -20> color White }"
!SIMULATION BOX
 WRITE(11,10003) -1.d0*L,-1.d0*L,1.d0*L,L,-1.d0*L,1.d0*L
 WRITE(11,10003) -1.d0*L,L,1.d0*L,L,L,1.d0*L
 WRITE(11,10003) -1.d0*L,-1.d0*L,1.d0*L,-1.d0*L,L,1.d0*L
 WRITE(11,10003) L,L,1.d0*L,L,-1.d0*L,1.d0*L
 WRITE(11,10003) -1.d0*L,-1.d0*L,-1.d0*L,-1.d0*L,-1.d0*L,1.d0*L
 WRITE(11,10003) -1.d0*L,L,1.d0*L,-1.d0*L,L,-1.d0*L
 WRITE(11,10003) -1.d0*L,-1.d0*L,-1.d0*L,-1.d0*L,L,-1.d0*L
 WRITE(11,10003) -1.d0*L,L,-1.d0*L,L,L,-1.d0*L
 WRITE(11,10003) -1.d0*L,-1.d0*L,-1.d0*L,L,-1.d0*L,-1.d0*L
 WRITE(11,10003) L,L,-1.d0*L,L,-1.d0*L,-1.d0*L
 WRITE(11,10003) L,L,1.d0*L,L,L,-1.d0*L
 WRITE(11,10003) L,-1.d0*L,1.d0*L,L,-1.d0*L,-1.d0*L
...
10003  FORMAT("cylinder{ <",F9.4,",",F9.4,",",F9.4,">, &
       <",F9.4,",",F9.4,",",F9.4,">, 0.15 pigment { color &
